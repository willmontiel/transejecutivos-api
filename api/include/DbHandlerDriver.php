...
class DbHandlerDriver {
  ...
  public function searchPendingService($code) {
    ...
    $stmt = $this->conn->prepare("SELECT o.id, o.fecha_s, o.hora_s1, o.hora_s2, s.hora1, s.hora2
                                    FROM orden AS o 
  ...
  /**
   * Busca y retorna un array con los datos de un servicio por id
   * @param type $id
   * @param type $code
   * @return string
   */
  ...
  /**
   * Crea el seguimiento de un servicio antiguo
   * @param type $id
   * @param type $code
   * @param type $start
   * @param type $end
...
//        $log = new LoggerHandler();
    ...
    $this->setTrace($reference, $start, $end, $user, $observations);
  ...
  private function validateServiceExists($id, $code) {
    ...
    $stmt->close();
    throw new InvalidArgumentException('Ocurrió un error, por favor contacta al administrador');
  }
  ...
  private function setTrace($reference, $start, $end, $user, $observations) {
    $conductor = "{$user['name']} {$user['lastname']}";
    ...
    if ($this->validateTraceExists($reference)) {
      $this->setExistTrace($reference, $start, $end, $user, $observations);
    } else {
  ...
  private function setExistTrace($reference, $start, $end, $user, $observations) {
    $times = "";
    $start = trim($start);
    $end = trim($end);
    ...
    $stmt = $this->conn->prepare("UPDATE seguimiento SET {$times} conductor = ?, elaborado = ?, observaciones = ? WHERE referencia = ?");
